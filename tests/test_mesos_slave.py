    ...
    def test_checks(self):
        ...
        with patch.object(klass, '_get_state', _mocked_get_state):
            with patch.object(klass, '_get_stats', _mocked_get_stats):
                ...
                time.sleep(1)
                self.run_check(config)
                [self.assertMetric(v[0]) for k, v in check.STATE_METRICS.iteritems()]
                [self.assertMetric(v[0]) for k, v in check.TASK_METRICS.iteritems()]
                [self.assertMetric(v[0]) for k, v in check.STATS_METRICS.iteritems()]
                self.assertMetric('mesos.state.task.status')
